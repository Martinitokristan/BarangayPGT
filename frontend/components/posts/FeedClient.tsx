...
// Interactive layer for the Main Feed.
// Receives `initialPosts` from the Server Component for instant loading.
// Handles its own API polling, search updates, and pagination locally.
...
function FeedContent({ initialPosts, initialLastPage }: FeedClientProps) {
    const { user } = useAuth();
    ...
    const [loading, setLoading] = useState(initialPosts.length === 0); // Show loading if no initial data
    const [search, setSearch] = useState(searchParams.get("search") || "");
    const [page, setPage] = useState(1);
    const [lastPage, setLastPage] = useState(initialLastPage);

    // Track if we are on the very first mount so we don't double-fetch page 1
    const [isFirstLoad, setIsFirstLoad] = useState(true);

    // Use a ref for posts length check to avoid recreating fetchPosts on every data change
    const postsRef = React.useRef<any[]>(initialPosts);
    ...
    // Poll for new posts every 60 seconds (silent background refresh) ONLY on page 1
    useEffect(() => {
        if (page === 1 && !search) {
    ...
    return (
        <div className="feed-container modern-feed">
            <div className="feed-header modern-feed-header"></div>
